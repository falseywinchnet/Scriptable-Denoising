...
Fixed native primitives also implement padded recursive mask smoothing and DIP
packet transport. The loader supplies typed addresses; their Python wrappers
...
config: float64[5] = sample rate, receive bandwidth Hz, squelch, harmonics,
...
# 0 = legacy Cleanup; 1 = optional DIP comb continuation, controlled by Harmonics.
CHAIN = (0, 1)
VAD_THRESHOLD = 0.057
MASK_STRENGTH = 1.0
...
# DIP COMB EXPERIMENT — fixed storage, native partial Fourier stages.
# E residue rows, Q periodic coordinates per row. Members k=delta+m*E share
# a packet. Change the width here and Reload to experiment with another lattice.
DIP_PACKET_WIDTH = 64
DIP_E = max(1, FFT_SIZE // DIP_PACKET_WIDTH)
DIP_Q = FFT_SIZE // DIP_E
DIP_HALF_BIN = int(BINS == FFT_SIZE // 2)
DIP_ANGLES = 2. * np.pi * np.arange(FFT_SIZE)[:, None] * np.arange(DIP_Q)[None, :] / FFT_SIZE
DIP_COS = np.ascontiguousarray(np.cos(DIP_ANGLES))
DIP_SIN = np.ascontiguousarray(np.sin(DIP_ANGLES))
CENTROID_COMFORT_GAIN = 0.0  # opt-in experiment: .035; amplitude relative to upper envelope
HARMONIC_MODEL = "packet"  # "packet" reference or "excitation" experiment
EXCITATION_COS = DIP_COS
EXCITATION_SIN = DIP_SIN
if HARMONIC_MODEL == "excitation":
    excitation_angles = 2.*np.pi*np.arange(FFT_SIZE)[:,None]*np.arange(FFT_SIZE)[None,:]/FFT_SIZE
    EXCITATION_COS = np.ascontiguousarray(np.cos(excitation_angles))
    EXCITATION_SIN = np.ascontiguousarray(np.sin(excitation_angles))
HARMONIC_LIMIT_HZ = 7600.
HARMONIC_FADE_HZ = 350.
HARMONIC_DECAY = 1.35
# Envelope experiment: retain the DIP coordinates, but stop repeating the
# donor's whole envelope at slowly decaying levels across the entire high band.
HARMONIC_MEMBER_DECAY = 1.1  # amplitude exp(-a*j-b*j*j), j beyond first member
HARMONIC_MEMBER_CURVATURE = 0.18
HARMONIC_PRIMARY_GAIN = 0.18
HARMONIC_EDGE_FLOOR_RATIO = 0.08  # contextual power relative to passband median
HARMONIC_JOIN_BINS = 2.0
HARMONIC_MAX_POWER_RATIO = 0.12
HARMONIC_MIX_SECONDS = 0.025
HARMONIC_PEAK_MIN_SECONDS = 0.025
HARMONIC_PEAK_MAX_SECONDS = 0.20
HARMONIC_HOLE_DEPTH = 0.75
HARMONIC_HOLE_WIDTH_HZ = 140.
HARMONIC_BACKFILL = 0.12
HARMONIC_STORAGE = 64 + 2 * PLANE + 2 * FRAMES * DIP_E + 5 * BINS + 4 * FFT_SIZE
ENTITY_STORAGE = max(ENTITY_STORAGE, HARMONIC_STORAGE, 64+5*PLANE+2*FRAMES*DIP_E+2*FRAMES+5*BINS+4*FFT_SIZE)
...
# HARMONICS ENTITY — no Python or allocation in the packet transport helper.
@njit(F64(F64), nogil=True, cache=False)
def softstep(value):
    x = min(1., max(0., value))
    return x * x * (3. - 2. * x)


@njit(F64(I64, I64, I64), nogil=True, cache=False)
def lattice_noise(time_cell, band, seed):
    # Integer hash: deterministic across callback sizes and overlapping contexts.
    x = (time_cell * 374761393 + band * 668265263 + seed * 1442695041) & 0x7fffffff
    x = ((x ^ (x >> 13)) * 1274126177) & 0x7fffffff
    return 2. * ((x ^ (x >> 16)) & 0x7fffffff) / 2147483647. - 1.


@njit(F64(F64, I64, I64), nogil=True, cache=False)
def coherent_noise(position, band, seed):
    cell = int(np.floor(position))
    fraction = softstep(position - cell)
    return lattice_noise(cell, band, seed) * (1. - fraction) + lattice_noise(cell + 1, band, seed) * fraction


@njit(VOID(MAT, MAT, MAT, F64, F64), nogil=True, cache=False)
def centroid_erosion(energy, smoothed, holes, sample_rate, protected_hz):
    """Soft ellipses at trough centroids BETWEEN supported temporal peaks.

    Time radius follows peak spacing; frequency radius spans nearby members.
    Flat sustained energy creates no arbitrary holes. A broad temporal gap is
    not bridged, and the original measured lower band is never touched.
    """
    holes[:] = 0.
    minimum = max(2, int(HARMONIC_PEAK_MIN_SECONDS * sample_rate / HOP))
    maximum = max(minimum + 1, int(HARMONIC_PEAK_MAX_SECONDS * sample_rate / HOP))
    df = sample_rate / FFT_SIZE
    sigma_f = max(0.6, HARMONIC_HOLE_WIDTH_HZ / df)
    for row in range(DIP_E):
        for t in range(FRAMES):
            value = 2. * energy[t, row]
            if t > 0:
                value += energy[t - 1, row]
            if t + 1 < FRAMES:
                value += energy[t + 1, row]
            smoothed[t, row] = value * 0.25
        previous = -1
        for t in range(1, FRAMES - 1):
            peak = smoothed[t, row]
            if peak <= 1e-24 or peak <= smoothed[t - 1, row] or peak < smoothed[t + 1, row]:
                continue
            if previous < 0:
                previous = t
                continue
            separation = t - previous
            if separation < minimum:
                if peak > smoothed[previous, row]:
                    previous = t
                continue
            if separation <= maximum:
                mass = moment = deepest = 0.
                for j in range(previous + 1, t):
                    f = (j - previous) / separation
                    roof = smoothed[previous, row] * (1. - f) + peak * f
                    deficit = max(0., 1. - smoothed[j, row] / max(roof, 1e-24))
                    mass += deficit
                    moment += j * deficit
                    deepest = max(deepest, deficit)
                if mass > 0.05 * separation:
                    center = moment / mass
                    sigma_t = max(1., 0.24 * separation)
                    depth = HARMONIC_HOLE_DEPTH * deepest
                    for member in range(row, BINS, DIP_E):
                        frequency = (member + .5 * DIP_HALF_BIN) * df
                        if frequency <= protected_hz or frequency >= HARMONIC_LIMIT_HZ:
                            continue
                        low = max(0, member - int(3. * sigma_f) - 1)
                        high = min(BINS, member + int(3. * sigma_f) + 2)
                        for j in range(previous + 1, t):
                            time_decay = ((j - center) / sigma_t) ** 2
                            for b in range(low, high):
                                ellipse = depth * np.exp(-.5 * (time_decay + ((b - member) / sigma_f) ** 2))
                                holes[j, b] = max(holes[j, b], ellipse)
            previous = t


@njit(I64(MAT, I64, I64, VEC, VEC), nogil=True, cache=False)
def occupied_edge(magnitude, low, high, scratch, sort_stack):
    """Conservative empty-tail detector; never treats a formant valley as edge.

    Require broadband support for a nonzero median, then retain every bin
    supported anywhere in the contextual mean. Search only the upper third of
    the configured band. This is a receiver-edge heuristic, not speech VAD.
    """
    ...
    for b in range(low, high+1):
        power = 0.
        for t in range(magnitude.shape[0]):
            power += magnitude[t,b]**2
        scratch[b] = power / magnitude.shape[0]
        scratch[bins+b-low] = scratch[b]
    sort_values(scratch[bins:bins+n], (scratch[bins:bins+n]).size, sort_stack)
    floor = median_sorted(scratch[bins:bins+n], n)
    peak = scratch[bins+n-1]
    if floor <= max(1e-24, peak*1e-5):
        return high
    threshold = HARMONIC_EDGE_FLOOR_RATIO * floor
    minimum = max(low, int(.65*high))
    for b in range(high, minimum-1, -1):
        if scratch[b] >= threshold:
            return high if high-b < 3 else b
    return high


@njit(I32(CUBE, CUBE, VEC, VEC), nogil=True, cache=False)
def harmonics(analysis, content, state, config):
    sort_stack = state[SORT_STACK_OFFSET:SORT_STACK_OFFSET + SORT_STACK_SIZE]
    enabled = config[3] != 0.
    mix = state[0]
    if not enabled and mix == 0.:
        return PASSTHROUGH
    rate = config[0]
    df = rate / FFT_SIZE
    protected = min(config[1], rate * .5)
    upper = min(HARMONIC_LIMIT_HZ, rate * .47)
    if upper <= protected + HARMONIC_FADE_HZ:
        return PASSTHROUGH
    lowbin = max(1, int(np.ceil(180. / df - .5 * DIP_HALF_BIN)))
    last = min(BINS - 2, int(np.floor(protected / df - .5 * DIP_HALF_BIN)))
    if last <= lowbin:
        return PASSTHROUGH
    offset = 64
    magnitude = state[offset:offset + PLANE].reshape((FRAMES, BINS)); offset += PLANE
    holes = state[offset:offset + PLANE].reshape((FRAMES, BINS)); offset += PLANE
    energy = state[offset:offset + FRAMES * DIP_E].reshape((FRAMES, DIP_E)); offset += FRAMES * DIP_E
    smoothed = state[offset:offset + FRAMES * DIP_E].reshape((FRAMES, DIP_E)); offset += FRAMES * DIP_E
    donors = state[offset:offset + BINS]; offset += BINS
    weights = state[offset:offset + 2 * BINS].reshape((BINS, 2)); offset += 2 * BINS
    addition = state[offset:offset + 2 * BINS].reshape((BINS, 2)); offset += 2 * BINS
    scratch = state[offset:offset + 4 * FFT_SIZE]
    energy[:] = 0.
    for t in range(FRAMES):
        for b in range(BINS):
            magnitude[t, b] = np.hypot(analysis[t, b, 0], analysis[t, b, 1])
    # Infer an empty receiver tail from the PRE-Cleanup contextual spectrum.
    # Sparse isolated tones are insufficient evidence to lower the boundary.
    nominal_last = last
    last = occupied_edge(magnitude, lowbin, nominal_last, scratch, sort_stack)
    if last < nominal_last:
        protected = (last + .5 * DIP_HALF_BIN) * df
    state[4] = protected
    for t in range(FRAMES):
        for b in range(lowbin, last + 1):
            energy[t, b % DIP_E] += magnitude[t, b] ** 2
    centroid_erosion(energy, smoothed, holes, rate, protected)
    step = HOP / (rate * HARMONIC_MIX_SECONDS)
    total_added = total_low = 0.
    for t in range(FIRST, FIRST + EMIT):
        mix = min(1., mix + step) if enabled else max(0., mix - step)
        if mix == 0.:
            continue
        donors[:] = -1.
        weights[:] = 0.
        low_power = 0.
        # Absolute frame origin keeps carrier transport coherent across Reload's
        # fresh history and across bypass/skip periods and host callback sizes.
        origin = config[4] + t * HOP - FFT_SIZE / 2.
        frame_center = config[4] + t * HOP
        for b in range(lowbin, last + 1):
            low_power += content[t, b, 0] ** 2 + content[t, b, 1] ** 2
        if low_power <= 1e-24:
            continue
        for k in range(last + 1, BINS - (1 - DIP_HALF_BIN)):
            frequency = (k + .5 * DIP_HALF_BIN) * df
            if frequency <= protected or frequency >= upper:
                continue
            donor = -1
            best = 0.
            # Existing lower members of THIS packet choose the periodic donor.
            for b in range(k % DIP_E, last + 1, DIP_E):
                if b < lowbin:
                    continue
                amplitude = np.hypot(content[t, b, 0], content[t, b, 1])
                neighbor = .5 * (magnitude[t, b - 1] + magnitude[t, b + 1])
                prominence = max(0., (magnitude[t, b] - neighbor) / (magnitude[t, b] + neighbor + 1e-24))
                score = amplitude * prominence
                if score > best:
                    best = score
                    donor = b
            if donor < 0 or best <= np.sqrt(low_power / (last - lowbin + 1)) * .035:
                continue
            source_amplitude = np.hypot(content[t, donor, 0], content[t, donor, 1])
            confidence = min(1., best / (source_amplitude + 1e-24))
            first_member = last + 1 + ((k % DIP_E - (last + 1) % DIP_E) % DIP_E)
            continuation = (k - first_member) / DIP_E
            # Stronger modulation follows measured packet energy, not an LFO.
            local = smoothed[t, k % DIP_E]
            slow = 0.
            start, stop = max(0, t-8), min(FRAMES, t+9)
            for j in range(start, stop):
                slow += smoothed[j, k % DIP_E]
            slow /= stop-start
            envelope = min(1.8, max(.35, (local/max(slow,1e-24))**.4))
            taper = softstep((frequency - protected) / (HARMONIC_JOIN_BINS*df)) * softstep((upper - frequency) / HARMONIC_FADE_HZ)
            # The primary has a distinct envelope. Higher members lose power
            # rapidly instead of repeating the same formant pattern everywhere.
            decay = np.exp(-HARMONIC_MEMBER_DECAY*continuation-HARMONIC_MEMBER_CURVATURE*continuation*continuation)
            gain = HARMONIC_PRIMARY_GAIN * confidence * envelope * taper * decay * (1. - holes[t, k])
            # Existing upper content reduces the amount synthesized into it.
            existing = np.hypot(content[t, k, 0], content[t, k, 1])
            gain *= max(0., 1. - existing / (source_amplitude * gain + 1e-24))
            # bfft coefficients use centered frames; carrier phase must use
            # that same origin. Leading-edge phase flips odd-bin translations.
            phase = 2. * np.pi * ((k - donor) * (frame_center % FFT_SIZE) / FFT_SIZE)
            # Soft, reproducible irregularity evolves over time; it is not
            # redrawn on every frame or independently for overlapping blocks.
            nr = coherent_noise(origin / (rate * .035), k // 2, 11)
            ni = coherent_noise(origin / (rate * .043), k // 2, 29)
            texture = HARMONIC_BACKFILL * np.sqrt((frequency - protected) / (upper - protected))
            real = 1. - texture + texture * nr
            imag = texture * ni
            c, s = np.cos(phase), np.sin(phase)
            donors[k] = donor
            weights[k, 0] = gain * (real * c - imag * s)
            weights[k, 1] = gain * (real * s + imag * c)
        code = _dip_transport(content[t].ctypes, donors.ctypes, weights.ctypes, addition.ctypes,
                              scratch.ctypes, DIP_COS.ctypes, DIP_SIN.ctypes,
                              FFT_SIZE, DIP_E, DIP_HALF_BIN)
        if code != 0:
            return ERROR
        added_power = 0.
        for k in range(last + 1, BINS):
            added_power += addition[k, 0] ** 2 + addition[k, 1] ** 2
        limit = np.sqrt(HARMONIC_MAX_POWER_RATIO * low_power / max(added_power, 1e-24))
        strength = min(1., limit) * softstep(mix)
        # Never overwrite protected measured coefficients, including roundoff
        # leakage from the inverse/forward packet walk.
        for k in range(last + 1, BINS):
            if (k + .5 * DIP_HALF_BIN) * df > protected:
                content[t, k, 0] += strength * addition[k, 0]
                content[t, k, 1] += strength * addition[k, 1]
        total_added += added_power * strength * strength
        total_low += low_power
    state[0] = mix
    state[1] += 1.
    state[2] = total_added
...
@njit(F64(MAT, I64, F64, F64), nogil=True, cache=False)
def guide_sample(guide, row, hz, rate):
    x=hz*2.*(GUIDE_FFT-1)/rate
    lo=min(guide.shape[1]-1,max(0,int(x)));hi=min(guide.shape[1]-1,lo+1)
    f=min(1.,max(0.,x-lo))
    return guide[row,lo]*(1.-f)+guide[row,hi]*f


@njit(PAIR(MAT, I64, F64, F64, F64), nogil=True, cache=False)
def excitation_pitch(guide, row, rate, edge, previous):
    """Shared spacing from several peaks and their half-period gaps.

    Off-comb samples penalize octave doubling; averaging all expected members
    penalizes subharmonic proposals supported only by every second member.
    This is optional synthesis periodicity evidence, not the denoiser's gate.
    """
    best=-1.;pitch=0.;confidence=0.
    high=min(edge-100.,2400.)
    for candidate in range(70,361,2):
        first=max(1,int(np.ceil(180./candidate)));last=min(39,int(high/candidate))
        if last-first<3:continue
        score=0.;hits=0
        for h in range(first,last+1):
            peak=guide_sample(guide,row,h*candidate,rate)
            gap=.5*(guide_sample(guide,row,(h-.5)*candidate,rate)+guide_sample(guide,row,(h+.5)*candidate,rate))
            prominence=(peak-gap)/(peak+gap+1e-24)
            score+=prominence
            if prominence>.2:hits+=1
        score/=last-first+1
        if hits<3:continue
        rank=score
        if previous>0.:rank-=.08*min(1.,abs(np.log(candidate/previous)))
        if rank>best:best=rank;pitch=float(candidate);confidence=max(0.,score)
    if confidence<.12:return 0.,0.
    return pitch,min(1.,confidence)


@njit(PAIR(F64), nogil=True, cache=False)
def harmonic_window_response(delta):
    """Exact centered symmetric-Hann response, normalized to coherent gain.

    Analytical finite-sum kernel: deposits fractional-frequency components on
    the existing synthesis grid. No new FFT or inferred high-band measurement.
    """
    omega=2.*np.pi*delta/FFT_SIZE
    a=2.*np.pi/(FFT_SIZE-1.)
    total=0.
    for j in range(3):
        w=omega if j==0 else omega+a if j==1 else omega-a
        den=np.sin(w*.5)
        d=float(FFT_SIZE) if abs(w)<1e-12 else np.sin(FFT_SIZE*w*.5)/den
        total+=(.5 if j==0 else .25)*d
    total/=((FFT_SIZE-1.)*.5)
    return total*np.cos(omega*.5),-total*np.sin(omega*.5)


@njit(PAIR(CUBE, CUBE, MAT, MAT, MAT, F64, F64, I64, I64, F64, BOOL), nogil=True, cache=False)
def centroid_comfort(content, original, envelope, holes, scratch, rate, origin, lowbin, last, mix_start, enabled):
    """Small aperiodic fill only inside existing centroid erosion regions.

    Independent complex samples on the STFT lattice fill between partials too.
    Absolute-time keys preserve callback determinism. The envelope and measured
    lower content set level; zero lower content or no hole means no addition.
    Reserve 10% of the addition's amplitude budget, 1% of its power budget.
    """
    df=rate/FFT_SIZE;protected=(last+.5*DIP_HALF_BIN)*df
    upper=min(HARMONIC_LIMIT_HZ,rate*.47);total=low_total=0.
    for t in range(FIRST,FIRST+EMIT):
        low_power=raw_power=0.
        for b in range(lowbin,last+1):
            low_power+=content[t,b,0]**2+content[t,b,1]**2
        low_total+=low_power
        # Envelope RMS measures scale; the cap prevents small envelope estimates
        # from amplifying a weak retained band.
        for b in range(lowbin,last+1):raw_power+=envelope[t,b]**2
        retained=min(1.,np.sqrt(low_power/max(raw_power,1e-24)))
        level=CENTROID_COMFORT_GAIN*envelope[t,max(lowbin,last-1)]*retained
        scratch[:]=0.;power=0.
        center=origin+t*HOP
        for b in range(last+1,BINS-(1-DIP_HALF_BIN)):
            hz=(b+.5*DIP_HALF_BIN)*df
            if hz>=upper:continue
            taper=softstep((hz-protected)/(2.*df))*softstep((upper-hz)/HARMONIC_FADE_HZ)
            amplitude=level*(hz/protected)**-1.5*taper*np.sqrt(max(0.,holes[t,b]))
            # Circular complex Gaussian samples: independent at each absolute
            # hop center. No shared harmonic phase or slowly held bin carrier.
            cell=int(np.floor(center/HOP))
            u=max(1e-12,.5*(1.+lattice_noise(cell,b,211)))
            angle=np.pi*(1.+lattice_noise(cell,b,307))
            radius=np.sqrt(-np.log(u))
            re=amplitude*radius*np.cos(angle)
            im=amplitude*radius*np.sin(angle)
            scratch[b,0]=re;scratch[b,1]=im;power+=re*re+im*im
        budget=.01*HARMONIC_MAX_POWER_RATIO*low_power
        strength=min(1.,np.sqrt(budget/max(power,1e-24)))
        # Caller applies on/off ramp to noise as well as periodic continuation.
        mix=min(1.,max(0.,mix_start+(1. if enabled else -1.)*(t-FIRST+1)*HOP/(rate*HARMONIC_MIX_SECONDS)))
        strength*=softstep(mix)
        for b in range(last+1,BINS):
            content[t,b,0]+=strength*scratch[b,0];content[t,b,1]+=strength*scratch[b,1]
            dr=content[t,b,0]-original[t,b,0];di=content[t,b,1]-original[t,b,1]
            total+=dr*dr+di*di
    return total,low_total


@njit(I32(CUBE, CUBE, MAT, VEC), nogil=True, cache=False)
def harmonics_excitation(analysis, content, all_state, config):
    """Pitch-linked excitation continuation; protected lower content is exact.

    Smooth envelope and periodic excitation are estimated separately. Shared
    source timing advances all partials, with measured lower residual phases.
    An E=1 DIP packet transports the resulting mixture; fixed residue spacing
    is not mistaken for vocal pitch. High envelope remains an explicit prior.
    """
    state=all_state[1];enabled=config[3]!=0.;mix=state[0]
    sort_stack = state[SORT_STACK_OFFSET:SORT_STACK_OFFSET + SORT_STACK_SIZE]
    if not enabled and mix==0.:return PASSTHROUGH
    rate=config[0];df=rate/FFT_SIZE;protected=min(config[1],rate*.5)
    upper=min(HARMONIC_LIMIT_HZ,rate*.47)
    lowbin=max(1,int(np.ceil(180./df-.5*DIP_HALF_BIN)))
    last=min(BINS-2,int(np.floor(protected/df-.5*DIP_HALF_BIN)))
    if last<=lowbin or upper<=protected:return PASSTHROUGH
    offset=64
    mag=state[offset:offset+PLANE].reshape((FRAMES,BINS));offset+=PLANE
    envelope=state[offset:offset+PLANE].reshape((FRAMES,BINS));offset+=PLANE
    holes=state[offset:offset+PLANE].reshape((FRAMES,BINS));offset+=PLANE
    energy=state[offset:offset+FRAMES*DIP_E].reshape((FRAMES,DIP_E));offset+=FRAMES*DIP_E
    smoothed=state[offset:offset+FRAMES*DIP_E].reshape((FRAMES,DIP_E));offset+=FRAMES*DIP_E
    pitches=state[offset:offset+FRAMES];offset+=FRAMES
    confidences=state[offset:offset+FRAMES];offset+=FRAMES
    donors=state[offset:offset+BINS];offset+=BINS
    weights=state[offset:offset+2*BINS].reshape((BINS,2));offset+=2*BINS
    addition=state[offset:offset+2*BINS].reshape((BINS,2));offset+=2*BINS
    scratch=state[offset:offset+4*FFT_SIZE];offset+=4*FFT_SIZE
    original=state[offset:offset+2*PLANE].reshape((FRAMES,BINS,2))
    if CENTROID_COMFORT_GAIN>0.:original[:]=content
    mix_start=mix
    guide=all_state[GUIDE_SLOT,GUIDE_OFFSET:GUIDE_OFFSET+GUIDE_PLANE].reshape((GUIDE_FRAMES,GUIDE_BINS))
    for t in range(FRAMES):
        for b in range(BINS):mag[t,b]=np.hypot(analysis[t,b,0],analysis[t,b,1])
    last=occupied_edge(mag,lowbin,last,scratch, sort_stack)
    protected=(last+.5*DIP_HALF_BIN)*df
    state[4]=protected
    previous=state[10]
    for t in range(FRAMES):
        row=min(GUIDE_FRAMES-1,int(round(t*HOP/float(GUIDE_HOP))))
        pitch,confidence=excitation_pitch(guide,row,rate,protected,previous)
        pitches[t],confidences[t]=pitch,confidence
        if pitch>0.:previous=pitch
        radius=max(2,int(pitch/df))
        total=0.
        for b in range(lowbin,last+1):total+=mag[t,b]**2
        for r in range(DIP_E):energy[t,r]=total/DIP_E
        for b in range(lowbin,last+1):
            lo=max(lowbin,b-radius);hi=min(last,b+radius)
            value=0.
            for k in range(lo,hi+1):value+=np.log(max(mag[t,k],1e-24))
            envelope[t,b]=np.exp(value/(hi-lo+1))
    # Shared temporal energy drives the existing between-peak erosion geometry.
    centroid_erosion(energy,smoothed,holes,rate,protected)
    step=HOP/(rate*HARMONIC_MIX_SECONDS);added_total=low_total=0.
    pitch_sum=conf_sum=texture_sum=count=0.
    phase=state[9];previous=state[10];previous_center=state[11];valid=state[12]!=0. and mix>0.
    for t in range(FIRST,FIRST+EMIT):
        mix=min(1.,mix+step) if enabled else max(0.,mix-step)
        pitch,confidence=pitches[t],confidences[t]
        center=config[4]+t*HOP
        if pitch<=0. or mix==0.:
            valid=False
            continue
        donor=-1;best=0.;excitation=0.;mass=0.;low_power=0.
        for b in range(lowbin,last+1):low_power+=content[t,b,0]**2+content[t,b,1]**2
        if low_power<=1e-24:valid=False;continue
        first=max(1,int(np.ceil(180./pitch)));last_h=min(39,int((protected-100.)/pitch))
        for h in range(first,last_h+1):
            b=min(last,max(lowbin,int(round(h*pitch/df-.5*DIP_HALF_BIN))))
            amp=np.hypot(content[t,b,0],content[t,b,1])
            if amp>best:best=amp;donor=b
            ratio=amp/max(envelope[t,b],1e-24)
            excitation+=min(4.,ratio);mass+=1.
        if donor<0 or mass<3. or best<=1e-18:valid=False;continue
        excitation/=mass
        if not valid:
            order=max(1,int(round((donor+.5*DIP_HALF_BIN)*df/pitch)))
            phase=np.arctan2(content[t,donor,1],content[t,donor,0])/order
            for h in range(first,last_h+1):
                b=min(last,max(lowbin,int(round(h*pitch/df-.5*DIP_HALF_BIN))))
                residual=np.arctan2(content[t,b,1],content[t,b,0])-h*phase
                state[16+h]=np.arctan2(np.sin(residual),np.cos(residual))
        else:
            phase+=2.*np.pi*.5*(pitch+previous)*(center-previous_center)/rate
            correction=weight=0.
            for h in range(first,last_h+1):
                b=min(last,max(lowbin,int(round(h*pitch/df-.5*DIP_HALF_BIN))))
                amp=np.hypot(content[t,b,0],content[t,b,1])
                error=np.arctan2(content[t,b,1],content[t,b,0])-h*phase-state[16+h]
                error=np.arctan2(np.sin(error),np.cos(error))
                correction+=amp*error/h;weight+=amp
            phase+=.25*max(-.15,min(.15,correction/max(weight,1e-24)))
        phase=np.arctan2(np.sin(phase),np.cos(phase));valid=True
        previous,previous_center=pitch,center
        residual_r=residual_i=0.
        for h in range(first,last_h+1):
            b=min(last,max(lowbin,int(round(h*pitch/df-.5*DIP_HALF_BIN))))
            observed=np.arctan2(content[t,b,1],content[t,b,0])-h*phase
            error=observed-state[16+h]
            state[16+h]+=.05*np.arctan2(np.sin(error),np.cos(error))
            residual_r+=np.cos(state[16+h]);residual_i+=np.sin(state[16+h])
        residual=np.arctan2(residual_i,residual_r)
        # Separately extrapolate the smooth envelope in log frequency/amplitude.
        sx=sy=sxx=sxy=n=0.
        for b in range(max(lowbin,int(.5*last)),last+1):
            x=np.log((b+.5*DIP_HALF_BIN)*df);y=np.log(max(envelope[t,b],1e-24))
            sx+=x;sy+=y;sxx+=x*x;sxy+=x*y;n+=1.
        slope=max(-3.,min(-.5,(n*sxy-sx*sy)/max(n*sxx-sx*sx,1e-24)))
        edge_env=envelope[t,max(lowbin,last-1)]
        desired=weights;desired[:]=0.
        first_upper=int(np.floor(protected/pitch))+1
        last_upper=int(upper/pitch)
        periodicity=min(1.,confidence/.6)
        texture=.25*(1.-periodicity)
        for h in range(first_upper,last_upper+1):
            hz=h*pitch;coordinate=hz/df-.5*DIP_HALF_BIN
            continuation=h-first_upper
            decay=np.exp(-.12*continuation-.005*continuation*continuation)
            amplitude=HARMONIC_PRIMARY_GAIN*periodicity*excitation*edge_env*(hz/protected)**slope*decay
            amplitude*=softstep((hz-protected)/(2.*df))*softstep((upper-hz)/HARMONIC_FADE_HZ)
            theta=h*phase+residual
            nr=coherent_noise(center/(rate*.017),h,73);ni=coherent_noise(center/(rate*.023),h,137)
            re=amplitude*((1.-texture)*np.cos(theta)+texture*nr)
            im=amplitude*((1.-texture)*np.sin(theta)+texture*ni)
            for b in range(max(last+1,int(coordinate)-3),min(BINS-(1-DIP_HALF_BIN),int(coordinate)+5)):
                wr,wi=harmonic_window_response(coordinate-b)
                erosion=1.-holes[t,b]
                desired[b,0]+=(re*wr-im*wi)*erosion
                desired[b,1]+=(re*wi+im*wr)*erosion
        donors[:]=-1.
        dr,di=content[t,donor,0],content[t,donor,1];den=dr*dr+di*di
        for b in range(last+1,BINS-(1-DIP_HALF_BIN)):
            re,im=desired[b,0],desired[b,1]
            amplitude=np.hypot(re,im);existing=np.hypot(content[t,b,0],content[t,b,1])
            retain=max(0.,1.-existing/max(amplitude,1e-24))
            donors[b]=donor
            weights[b,0]=retain*(re*dr+im*di)/den
            weights[b,1]=retain*(im*dr-re*di)/den
        code=_dip_transport(content[t].ctypes,donors.ctypes,weights.ctypes,addition.ctypes,
                            scratch.ctypes,EXCITATION_COS.ctypes,EXCITATION_SIN.ctypes,FFT_SIZE,1,DIP_HALF_BIN)
        if code!=0:return ERROR
        power=0.
        for b in range(last+1,BINS):power+=addition[b,0]**2+addition[b,1]**2
        reserve=.81 if CENTROID_COMFORT_GAIN>0. else 1.
        strength=min(1.,np.sqrt(reserve*HARMONIC_MAX_POWER_RATIO*low_power/max(power,1e-24)))*softstep(mix)
        for b in range(last+1,BINS):
            content[t,b,0]+=strength*addition[b,0];content[t,b,1]+=strength*addition[b,1]
        added_total+=power*strength*strength;low_total+=low_power
        pitch_sum+=pitch;conf_sum+=confidence;texture_sum+=texture;count+=1.
    if CENTROID_COMFORT_GAIN>0.:
        added_total,low_total=centroid_comfort(content,original,envelope,holes,addition,rate,config[4],lowbin,last,mix_start,enabled)
    state[0]=mix;state[1]+=1.;state[2],state[3]=added_total,low_total
    state[5]=pitch_sum/max(1.,count);state[6]=conf_sum/max(1.,count);state[7]=count/EMIT;state[8]=texture_sum/max(1.,count)
    state[9],state[10],state[11],state[12]=phase,previous,previous_center,1. if valid else 0.
    return PROCESS
    ...
    def run(analysis, content, state, config):
        ...
        for slot in range(len(CHAIN)):
            if CHAIN[slot] == 0:
                ...
                action = harmonics_excitation(analysis,content,state,config) if HARMONIC_MODEL == "excitation" else harmonics(analysis, content, state[slot], config)
            else:
                return ERROR
            if action == SKIP or action == ERROR:
                for later in range(slot + 1, len(CHAIN)):
                    if CHAIN[later] == 1:
                        state[later, 0] = 0.
                return action
            if action == PROCESS:
