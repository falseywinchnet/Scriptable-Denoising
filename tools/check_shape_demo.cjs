...
(async()=>{
 ...
 await page.waitForFunction(()=>document.querySelector('#play').textContent==='Play'&&spectra.length===13&&spectra[12]?.surface_panels.every(p=>p.bytes),{timeout:60000});
 await page.locator('#choices button').getByText('Shape-weighted contour',{exact:true}).click();
 ...
 await page.locator('.visuals').screenshot({path:'build/listening-demo-v15/browser-shape.png'});
 for(const field of ['surface_mean','surface_variance','surface_low','surface_occupancy','surface_reference','surface_shape','surface_floor']){
  await page.locator('#surfaceField').selectOption(field);
  await page.waitForFunction(()=>!visualDirty);
  if(field==='surface_shape')await page.locator('.visuals').screenshot({path:'build/listening-demo-v15/browser-shape-score.png'});
 }
 for(const label of ['Occupancy + comfort','Shape + comfort']){
  ...
  await page.locator('.visuals').screenshot({path:'build/listening-demo-v15/browser-'+(label.startsWith('Occupancy')?'occupancy-comfort':'shape-comfort')+'.png'});
 }
 ...
 const snapshot=await page.evaluate(()=>({buttons:[...document.querySelectorAll('#choices button')].map(b=>b.textContent),playing,position:document.querySelector('#seek').value,panels:spectra.flatMap(s=>[...s.panels,...(s.surface_panels||[])].map(p=>({kind:p.kind,bytes:p.bytes.length,expected:p.bins*p.frames}))),harmonics:[11,12].map(i=>({name:variants[i][0],activeBlocks:spectra[i].statistics.filter(s=>s.harmonic?.[0]>0).length}))}));
 if(errors.length||snapshot.playing||snapshot.buttons.length!==13||snapshot.panels.some(p=>p.bytes!==p.expected)||snapshot.position!==stopped||snapshot.harmonics.some(h=>!h.activeBlocks))throw Error(JSON.stringify({errors,snapshot}));
 fs.writeFileSync('docs/evidence/shape-comfort-browser.json',JSON.stringify({errors,...snapshot},null,2));
